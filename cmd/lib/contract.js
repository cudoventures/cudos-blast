...
const Contract = class {
  ...
  async uploadContract () {
    ...
    return await this.client.upload(this.config.account.address0, wasm, uploadFee)
  }

  async initContract (codeId) {
    const instantiateFee = calculateFee(500_000, this.gasPrice)
    return await this.client.instantiate(this.config.account.address0, codeId, this.initMsg, this.label, instantiateFee)
  }
