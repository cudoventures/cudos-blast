...
function compileCmd(argv) {
    let optcmd = `docker run --rm -v "${process.env.PWD}/contracts/alpha/src/${argv.contractname}":/code  --mount type=volume,source="${argv.contractname}_cache",target=/code/target  --mount type=volume,source=registry_cache,target=/usr/local/cargo/registry cosmwasm/rust-optimizer:0.11.3`
    ...
    if (!fs.existsSync(`${process.env.PWD}contracts/alpha/src/${argv.contractname}`)) {
