...
| async instantiate(msg, codeId, options = { signer: null, label: null, funds: null }) | Instantiates an uploaded contract using its codeId. Can be used for undeployed as well as already deployed contracts. The new instantiated contract should be additionally accessed by its address.                                                                                             | const instantiateTx = await contract.instantiate(MSG_INIT)              |
