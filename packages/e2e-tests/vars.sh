...
export TESTS_FOLDER='./packages/e2e-tests/tests'
export INIT_FOLDER='./test-blast-init'
export PATH_TO_TEMPLATE='./packages/blast-core/template'
export CONTAINER_NAME='cudos_blast_node'
export DOCKER_ERROR='Cannot connect to the Docker daemon'
export LOCAL_NODE_EXEC='docker compose -f ./packages/blast-core/config/docker-compose-start.yaml -f ./packages/blast-core/config/docker-compose-init.yaml exec -T cudos-node'
export DOCKER_COMPOSE='docker compose -f ./packages/blast-core/config/docker-compose-start.yaml -f ./packages/blast-core/config/docker-compose-init.yaml'
red='\033[0;31m'
green='\033[0;32m'
reset_color='\033[m'
